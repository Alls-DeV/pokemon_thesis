...
class PolimiBot(Player):
    ...
    def get_active_pokemon_prompt(self, battle: AbstractBattle, opponent: bool, enhanced: bool) -> str:
        ...
        if not active_mon:
            return f"Information about {prefix} active pokemon: No active Pokemon found."
        ...
        if active_mon.ability:
            try:
                ...
                ability_effect = f" ({self.ability_effect[active_mon.ability]['effect']})"
            except:
        ...
        if not opponent and battle.available_moves:
            ...
            for move in battle.available_moves:
                if move:
                    ...
                    ko_turns = None
                    try:
                        ko_result = self.turns_to_ko(battle, move.id, attacker_is_opponent=opponent)
                        if isinstance(ko_result, int):
                            ko_turns = ko_result
                    except Exception:
                        ko_turns = None
                    ko_str = f" ({ko_turns} turns to KO opponent's pokemon)" if ko_turns is not None else ""
        ...
        # Get tera type
        tera_type = "Unknown"
        if hasattr(active_mon, '_terastallized_type') and active_mon._terastallized_type:
        ...
        if enhanced and self.bayesian_predictor and opponent:
            ...
            if 'abilities' in predictions and predictions['abilities'] and ability_name == "Unknown":
                ...
                if normalized_best_ability not in self.ability_effect:
                    print(f"[WARNING]: Predicted ability '{normalized_best_ability}' not found in ability_effect.json")
            ...
            if 'items' in predictions and predictions['items'] and item_name == "unknown_item":
                ...
                if normalized_best_item not in self.item_effect:
                    print(f"[WARNING]: Predicted item '{normalized_best_item}' not found in item_effect.json")
            ...
            if 'moves' in predictions and predictions['moves'] and len(moves_info) < 4:
                ...
                for move_name, prob in predicted_moves[:moves_to_add]:
                    ...
                    normalized_move_name = normalized_move_name.translate(str.maketrans('', '', string.punctuation))
                    if normalized_move_name not in known_moves:
                        if normalized_move_name not in self.move_effect:
                            print(f"[WARNING]: Predicted move '{normalized_move_name}' not found in move_effect.json")
                        move_effect = "" if normalized_move_name not in self.move_effect else f": {self.move_effect[normalized_move_name]}"
                        ...
                        try:
                            ...
                            ko_result = self.turns_to_ko(battle, pseudo_id, attacker_is_opponent=opponent)
                            if isinstance(ko_result, int):
                                ko_turns = ko_result
                        except Exception:
                            ko_turns = None
        ...
        if not opponent and battle.opponent_active_pokemon:
            speed_line = self.get_speed_prompt(active_mon, battle.opponent_active_pokemon)
            if speed_line:
...
{speed_line}"""
    ...
    def turns_to_ko(self, battle: AbstractBattle, move_id: str, gen: int = 9, attacker_is_opponent: bool = False, attacker_pokemon: Pokemon | None = None, defender_pokemon: Pokemon | None = None) -> int | dict:
        ...
        if not (js_dir / 'node_modules' / '@smogon' / 'calc' / 'package.json').exists():
            try:
                subprocess.run(['npm', 'install', '--no-audit', '--no-fund'], cwd=str(js_dir), check=True,
                               stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            except Exception as e:
                return {"error": f"npm install failed: {e}"}

        status_map = {
            'burnt': 'brn', 'paralyzed': 'par', 'poisoned': 'psn', 'toxic': 'tox', 'frozen': 'frz', 'sleeping': 'slp'
        }
        ...
        def build_side(battle: AbstractBattle, mon: Pokemon):
            ...
            evs = {'hp': int(ev_array[0]), 'atk': int(ev_array[1]), 'def': int(ev_array[2]),
                   'spa': int(ev_array[3]), 'spd': int(ev_array[4]), 'spe': int(ev_array[5])}
        ...
        def build_field(battle: AbstractBattle):
            ...
            any_side = any(field_obj['attackerSide'].values()) or any(field_obj['defenderSide'].values())
            if weather_name or terrain_name or any_side or is_trick_room or is_gravity or is_magic_room or is_wonder_room:
                return field_obj
            return None

        # Decide which side is attacker based on flag
        if attacker_is_opponent:
            attacker_mon = attacker_pokemon if attacker_pokemon else battle.opponent_active_pokemon
            defender_mon = defender_pokemon if defender_pokemon else battle.active_pokemon
            attacker_side = build_side(battle, attacker_mon)
            defender_side = build_side(battle, defender_mon)
        else:
            attacker_mon = attacker_pokemon if attacker_pokemon else battle.active_pokemon
            defender_mon = defender_pokemon if defender_pokemon else battle.opponent_active_pokemon
            attacker_side = build_side(battle, attacker_mon)
            defender_side = build_side(battle, defender_mon)
        move_name_for_calc = move_id.lower().replace(' ', '').replace('-', '')
        payload = { 'gen': gen, 'attacker': attacker_side, 'defender': defender_side, 'move': {'name': move_name_for_calc} }
        field_data = build_field(battle)
        if field_data:
            payload['field'] = field_data

        try:
            proc = subprocess.run([node_path, str(calc_script)], input=json.dumps(payload).encode('utf-8'),
                                  stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True, cwd=str(js_dir))
        except subprocess.CalledProcessError as e:
            return {"error": f"Node calc failed: {e.stderr.decode('utf-8', errors='ignore')}", 'payload': payload}
        except Exception as e:
        ...
        try:
            ...
            return {"error": f"Parse error: {e}", 'raw': proc.stdout.decode('utf-8', errors='ignore'), 'payload': payload}
    ...
    def get_side_conditions_prompt(self, battle: AbstractBattle) -> str:
        ...
        if player_side_condition_list:
            player_side_condition_prompt = "Your team's side conditions:\n" + "\n".join(player_side_condition_list) + "\n"
        ...
        if opponent_side_condition_list:
            opponent_side_condition_prompt = "Opponent team's side conditions:\n" + "\n".join(opponent_side_condition_list) + "\n"
    ...
    def get_player_team_prompt(self, battle: AbstractBattle) -> str:
        ...
        for pokemon in battle.team.values():
            if pokemon and pokemon.species:
                    ...
                    for move_id, move in pokemon.moves.items():
                        if move:
                            ...
                            try:
                                ...
                                print(f"[WARNING]: Move '{move.id}' not found in move_effect.json")
                            ...
                            if battle.opponent_active_pokemon:
                                try:
                                    ...
                                    ko_result = self.turns_to_ko(battle, move.id, attacker_is_opponent=False, attacker_pokemon=pokemon)
                                    if isinstance(ko_result, int):
                                        ko_turns = ko_result
                                except Exception:
                                    ko_turns = None
                            ...
                            ko_str = f" ({ko_turns} turns to KO opponent's pokemon)" if ko_turns is not None else ""
                            moves_list.append(f"    * {move_name}{move_explanation}{ko_str}")
                            ...
                            try:
                                ...
                                print(f"[WARNING]: Move '{move_id}' not found in move_effect.json")
                                move_explanation = ": Effect unknown"
                            ...
                            if battle.opponent_active_pokemon:
                                try:
                                    # Pass the team pokemon as the attacker to calculate damage from this pokemon
                                    ko_result = self.turns_to_ko(battle, move_id, attacker_is_opponent=False, attacker_pokemon=pokemon)
                                    if isinstance(ko_result, int):
                                        ko_turns = ko_result
                                except Exception:
                                    ko_turns = None
                ...
                speed_line = ""
                if battle.opponent_active_pokemon:
                    speed_comparison = self.get_speed_prompt(pokemon, battle.opponent_active_pokemon)
                    if speed_comparison:
                ...
                if battle.opponent_active_pokemon:
                    ...
                    if hasattr(battle.opponent_active_pokemon, 'moves') and battle.opponent_active_pokemon.moves:
                        for opp_move in battle.opponent_active_pokemon.moves.values():
                            if opp_move:
                                try:
                                    ...
                                    ko_result = self.turns_to_ko(battle, opp_move.id, attacker_is_opponent=True, 
                                                                attacker_pokemon=battle.opponent_active_pokemon,
                                                                defender_pokemon=pokemon)
                                    if isinstance(ko_result, int):
                                        if min_opponent_ko_turns is None or ko_result < min_opponent_ko_turns:
                                            min_opponent_ko_turns = ko_result
                                            fastest_opponent_move = self.denormalize_move_name(opp_move.id)
                                except Exception:
                                    pass
...
{opponent_ko_line}"""
    ...
    def get_terastallization_prompt(self, battle: AbstractBattle) -> str:
        ...
        if battle.can_tera:
            ...
            if hasattr(battle.active_pokemon, '_terastallized_type') and battle.active_pokemon._terastallized_type:
                active_tera_type = battle.active_pokemon._terastallized_type.name.capitalize()
            ...
            tera_type = battle.active_pokemon._terastallized_type.name.capitalize() if battle.active_pokemon._terastallized_type else "Unknown"
        ...
        if battle.opponent_active_pokemon:
            if battle.opponent_active_pokemon.terastallized:
                opp_tera_type = battle.opponent_active_pokemon._terastallized_type.name.capitalize() if battle.opponent_active_pokemon._terastallized_type else "Unknown"
                ...
                tera_prompt += f"* WARNING: Opponent can still terastallize this battle\n"
        
        return tera_prompt

    def get_system_prompt(self, battle: AbstractBattle) -> str:
        active_pokemon = battle.active_pokemon.species
        if active_pokemon not in self.strategy_data:
            print(f"Active Pokémon {active_pokemon} not found in strategy file.")
            return ""
        
        # TODO: Adjust the system prompt
        system_prompt = f"You are a pokemon battler that targets to win the pokemon battle. Your active Pokémon is {active_pokemon}, and you *MUST* follow this strategy:\n{self.strategy_data[active_pokemon]}\n"
    ...
    def choose_move(self, battle: AbstractBattle) -> BattleOrder:
        ...
        opponent_active_pokemon_prompt = self.get_active_pokemon_prompt(battle, opponent=True, enhanced=True)
        
        # Check if active Pokemon is fainted
        active_pokemon_fainted = (
            not battle.active_pokemon or 
            battle.active_pokemon.fainted or 
            self.check_status(battle.active_pokemon.status) == "fainted"
        )
        ...
        if active_pokemon_fainted:
            ...
            switch_prompt += opponent_active_pokemon_prompt + "\n" + player_team_prompt + "\n"
            switch_prompt += f"\nAvailable switches:\n{switches_options}\n"
...
Provide your response in JSON format with the following structure:
{
  ...
  "switch": "The name of the pokemon you want to switch to (must be one from the available switches list)"
            ...
            try:
                switch_data = json.loads(switch_response_raw)
                chosen_pokemon_name = switch_data.get("switch", "").strip()
                
                # Find matching pokemon
                for pokemon in battle.available_switches:
                    if pokemon.species.lower().replace(' ', '').replace('-', '') == chosen_pokemon_name.lower().replace(' ', '').replace('-', ''):
                        return BattleOrder(pokemon)
                
                # If no exact match found, try fuzzy matching
                available_switch_species = [p.species for p in battle.available_switches]
                matches = get_close_matches(chosen_pokemon_name.lower().replace(' ', '').replace('-', ''), 
                                          [s.lower().replace(' ', '').replace('-', '') for s in available_switch_species], 
                                          n=1, cutoff=0.6)
                if matches:
                    for pokemon in battle.available_switches:
                        if pokemon.species.lower().replace(' ', '').replace('-', '') == matches[0]:
                            return BattleOrder(pokemon)
            except Exception as e:
        ...
        player_active_pokemon_prompt = self.get_active_pokemon_prompt(battle, opponent=False, enhanced=False)
        player_team_prompt = self.get_player_team_prompt(battle)
        ...
        moves_options = "\n".join(available_moves_list) if available_moves_list else "  - No moves available"
        
        # Build move prompt with JSON output instructions
        move_prompt = ""
        if side_conditions_prompt:
            move_prompt += side_conditions_prompt + "\n"
        move_prompt += player_active_pokemon_prompt + "\n" + opponent_active_pokemon_prompt + "\n"
            ...
            move_prompt += """\nProvide your response in JSON format with the following structure:
{
  ...
  "move": "The name of the move you want to use (must be one from the available moves list)"
        ...
        if not has_available_switches:
            ...
            try:
                move_data = json.loads(move_response_raw)
                chosen_move_name = move_data.get("move", "").strip()
                should_terastallize = move_data.get("terastallize", False)
                
                # Find matching move
                for move in battle.available_moves:
                    if move.id.lower().replace(' ', '').replace('-', '') == chosen_move_name.lower().replace(' ', '').replace('-', ''):
                        return BattleOrder(move, terastallize=should_terastallize if battle.can_tera else False)
                
                # If no exact match found, try fuzzy matching
                available_move_ids = [m.id for m in battle.available_moves]
                matches = get_close_matches(chosen_move_name.lower().replace(' ', '').replace('-', ''), 
                                          [m.lower().replace(' ', '').replace('-', '') for m in available_move_ids], 
                                          n=1, cutoff=0.6)
                if matches:
                    for move in battle.available_moves:
        ...
        switch_prompt += """\nProvide your response in JSON format with the following structure:
{
  ...
  "switch": "The name of the pokemon you want to switch to (must be one from the available switches list)"
        ...
        opponent_species = self.denormalize_pokemon_name(battle.opponent_active_pokemon.species) if battle.opponent_active_pokemon else "Unknown"
        opponent_hp = round(battle.opponent_active_pokemon.current_hp_fraction * 100, 1) if battle.opponent_active_pokemon else 0
...
{{
  ...
  "choice": "move" or "switch"
        ...
        try:
            merger_data = json.loads(merger_response_json)
